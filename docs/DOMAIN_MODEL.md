...
Phase 7 `@Vimla` execution record. Unique `(userId, clientRequestId)` for idempotent create. Status includes planning, clarification, confirmation, executing, succeeded, failed, canceled, and partial. Steps store tool name and public card fields; `inputJson` is server-only. Write tools emit `OperatorAuditEvent`. Confirmation tokens are HMAC-hashed; plaintext is issued only while `AWAITING_CONFIRMATION`. The model never supplies `userId` or permissions — `ActorContext` comes from the session. Phase 9 adds `invocationScope` (`PERSONAL` | `DIRECT_CHAT`), optional `directConversationId`, and consent flags for an ephemeral context bundle. Direct Chat runs do not persist operator plaintext into AI `Message` rows.

## DirectConversation / DirectMessage / UserCryptoDevice
Phase 9 1:1 E2EE chats. `pairKey` is unique for two users. Server stores ciphertext envelopes, public device keys, signed/one-time prekeys, unread cursors, and `@Vimla` privacy consents. Private keys stay on devices. See `docs/DIRECT_CHATS.md`.
