...
- Phase 7 `@Vimla`: separate `@vimla/operator` planner/executor; typed tool registry; session `userId` is the only actor; model cannot set identity/permissions; destructive tools require confirmation; runs are idempotent on `clientRequestId`; planner JSON is never shown to the user. Capability defaults off (`OPERATOR_ENABLED=false`, `CONSUMER_FEATURES.vimlaOperator` unless `NEXT_PUBLIC_VIMLA_OPERATOR=true`). No Direct Chats, Auto Router, payments, or admin/security actions.
- Phase 8 Projects: `@vimla/projects` domain; owner plan is the billing/entitlement subject; `lastOpenedAt` ranking; reversible `PLAN_LOCKED` / member read-only states; invite HMAC tokens; no ownership transfer. Capability defaults off (`PROJECTS_ENABLED=false`, `CONSUMER_FEATURES.projects` unless `NEXT_PUBLIC_VIMLA_PROJECTS=true`).
- Phase 9 Direct Chats: E2EE 1:1 (`X3DH` + Double Ratchet, noble primitives) with ciphertext-only server storage; `@Vimla` invocation scope is the current Direct Chat; context is a client-decrypted ephemeral bundle gated by both participants’ consent. Capability defaults off (`DIRECT_CHATS_ENABLED=false`, `CONSUMER_FEATURES.directChats` unless `NEXT_PUBLIC_VIMLA_DIRECT_CHATS=true`). Not AES-at-rest. See `docs/DIRECT_CHATS.md`.
