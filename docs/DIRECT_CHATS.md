...
- Direct Chat Operator recovery is phase-durable. Non-terminal runs such as `AWAITING_CONFIRMATION` keep their parent intent after output delivery, so reload restores the Operator panel. Every recovery fetch stages the newest run snapshot without allowing an older `updatedAt` snapshot to regress it. Confirmation always fetches a fresh confirmation token immediately before confirm. All returned Operator actions are staged with stable local delivery keys; terminal parents are removed only after every encrypted response/action output has been durably delivered. A later ordinary send can also recover a previously committed `OPERATOR_INVOKE` without requiring reload/navigation.
