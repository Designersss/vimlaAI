...
- Every browser coordination lock is backed by the same IndexedDB lease namespace with bounded expiry/renewal. Web Locks, when available, are a non-waiting best-effort optimization (`ifAvailable`); durable IndexedDB lease acquisition remains the ownership source of truth, so a stuck external Web Lock cannot block stale-lease recovery. Versioned compare-and-swap remains the final ratchet correctness guard.
