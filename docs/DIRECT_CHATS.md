...
- First-time/offline decrypt processes messages oldest-first. Only an explicit missing-session result can trigger X3DH history backfill; authentication/corruption failures do not. Backfill is capped to eight older pages per attempt so a malformed or very deep history cannot force an unbounded fetch/decrypt/cache pass.
