...
- The pending X3DH marker is cleared only after successful server acknowledgment. An ambiguous response may therefore cause a later envelope to repeat already-persisted initiation metadata; an existing receiver session ignores that metadata, while the signed envelope remains authoritative.
