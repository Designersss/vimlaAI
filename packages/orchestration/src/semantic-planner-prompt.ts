...
export function buildSemanticPlannerPrompt(
  ...
  return [
    ...
    "Every explicit executor mention in PLANNER_MENTIONS must be consumed exactly once by a MENTION targetHint with semanticRole=EXECUTION.",
    "Never invent a model, provider, user id, permission, price, entitlement, or external executor.",
    "A MENTION targetHint may reference only a supplied occurrenceId and must never be substituted by an EVALUATOR node.",
    "Use EVALUATOR only for a distinct included evaluation node that does not replace an explicitly selected executor. It must declare exactly one JSON output and one or more acceptanceCriteria.",
