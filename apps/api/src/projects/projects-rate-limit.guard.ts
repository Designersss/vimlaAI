...
@Injectable()
export class ProjectsRateLimitGuard implements CanActivate {
  constructor(
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const allowed = await this.hit(
      `ratelimit:projects:user:${userId}`,
      this.config.projectsMutationLimitPerMinute,
    );
    if (!allowed) {
      throw new HttpException(
        { code: "rate_limited", message: "Too many project requests" },
  ...
  private async hit(key: string, max: number): Promise<boolean> {
    try {
      ...
      throw new HttpException("Projects rate limiter unavailable", HttpStatus.SERVICE_UNAVAILABLE);
    }
  }
}
