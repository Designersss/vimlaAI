...
export class NotificationRateLimitGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const allowed = await this.hit(
      `ratelimit:notifications:user:${userId}`,
      this.config.notifyInboxLimitPerMinute,
    );
    if (!allowed) {
      throw new HttpException(
        { code: "rate_limited", message: "Too many notification requests" },
  ...
  private async hit(key: string, max: number): Promise<boolean> {
    try {
      ...
      throw new HttpException("Notification rate limiter unavailable", HttpStatus.SERVICE_UNAVAILABLE);
    }
  }
}
