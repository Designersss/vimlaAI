...
@Injectable()
export class OperatorRateLimitGuard implements CanActivate {
  constructor(
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const allowed = await this.hit(
      `ratelimit:operator:user:${userId}`,
      this.config.operatorRateLimitPerMinute,
    );
    if (!allowed) {
      throw new HttpException(
        { code: "rate_limited", message: "Too many operator requests" },
  ...
  private async hit(key: string, max: number): Promise<boolean> {
    try {
      ...
      throw new HttpException("Operator rate limiter unavailable", HttpStatus.SERVICE_UNAVAILABLE);
    }
  }
}
